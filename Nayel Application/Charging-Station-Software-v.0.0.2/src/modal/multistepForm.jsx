...
const MultiStepForm = ({ open, handleClose }) => {
  ...
  // Add an event listener to listen for changes to "access_token" in localStorage
  ...
  //       if (tokenCheck && activeStep > 1) {
  //         const currentTime = Date.now();
  //         const tokenTime = localStorage.getItem("access_token_time");
  //         const tokenTimeout = 1 * 60 * 1000; // 3 minutes in milliseconds

  //         if (currentTime - tokenTime >= tokenTimeout) {
  //           localStorage.removeItem("access_token");
  //           localStorage.removeItem("access_token_time");
  //           setAccess_token(null); // Clear the access token
  //         }
  //       } else if (!tokenCheck) {
  //         setActiveStep(0);
  //         setAccessTimeoutMessage("Access Timeout. Please log in again.");
  ...
  //         }));
  //         setTimeout(() => {
  //           setAccessTimeoutMessage(null); // Clear the timeout message after 3 seconds
  //         }, 8000);
  //       }
  //       // console.log("in access_token");
  //     };

  //     const intervalId = setInterval(handleAccessToken, 1000);
